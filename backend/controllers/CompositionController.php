...
class CompositionController extends Controller
{
    ...
    {
        $id= $_POST['id'];
        $composition = Composition::findOne(['id'=>$id]);
        if($composition->is_active==1)
        {
            $composition->is_active = 0;
            $composition->save();
            if($composition->save(false))
            {
                Yii::$app->session->setFlash('success', 'successfully Inactive Composition');
            }
        }
        else
        {
            $composition->is_active=1;
            $composition->save();
            if($composition->save(false))
            {
                Yii::$app->session->setFlash('success', 'successfully active Composition');
            }
        }
    }
}

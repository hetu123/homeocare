            ...
            [
                ...
                'contentOptions' =>function ($model, $key, $index, $column)
                {
                    if($model->is_active=='1'){
                        return ['style' =>'width:100px','class'=>'btn btn-success','onclick'=>"ingredientActive($model->id)"];

                    }
                    else
                    {
                        return ['style' =>'width:100px','class'=>'btn btn-danger','onclick'=>"ingredientActive($model->id)"];
                    }
                ...
                'buttons' => [
                    ...
                    'delete' => function($url, $model){
                        return Html::a('<span class="glyphicon glyphicon-trash"></span>',
                            ['/ingredients/delete', 'id' => $model->id],
                            [
...
<script>
    function ingredientActive($id){
        var result = confirm("Are you sure ?");
        if(result) {
            $.ajax({
                url: '<?php echo Yii::$app->request->baseUrl . '/ingredients/active' ?>',
                type: 'post',
                data: {id: $id},
